...
const gethosbitalbyname = async(req,res)=>
{
    const hosbital_name =req.params.hospitalname;
    console.log(hosbital_name)
    try {
        const data=  await hosbital.find({hosbital_name});
    res.json({message:"success",data})
    } catch (error) {
        res.json({massage:"error",error})
    }
...
const updateHospitalHandler = async(req,res)=>{
    const hosbital_name =req.params.hospitalname;
    const {cancer,Emergency_beds}= req.body;
    const newHosbital = await hosbital.updateOne({hosbital_name},{"$set":{cancer,Emergency_beds}})
